				...
				if (CombatFactory.canAttack(npc, CombatFactory.getMethod(npc), player)) {
					if (bandits) {
						...
						if (Misc.getRandom(2) == 1) {
							String s = zammy > 0 ? "Zamorakian" : "Saradominist";
							if (s.equals("Zamorakian")) {
								npc.forceChat("Prepare to suffer, " + s + " scum!");
							} else if (s.equals("Saradominist")) {
								npc.forceChat("Time to die, " + s + " filth!");
							}
						}
					}
