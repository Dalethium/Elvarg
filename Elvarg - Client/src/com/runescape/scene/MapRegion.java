...
public final class MapRegion {
			...
			for (int z = 0; z < 4; z++) {
				...
				for (int j4 = 1; j4 < regionSizeY - 1; j4++) {
					for (int j5 = 1; j5 < regionSizeX - 1; j5++) {
						...
						int j17 = (shading[j5 - 1][j4] >> 2) + (shading[j5 + 1][j4] >> 3) + (shading[j5][j4 - 1] >> 2)
								+ (shading[j5][j4 + 1] >> 3) + (shading[j5][j4] >> 1);
				...
				for (int l6 = -5; l6 < regionSizeX + 5; l6++) {
					...
					if (l6 >= 1 && l6 < regionSizeX - 1) {
						...
						for (int k17 = -5; k17 < regionSizeY + 5; k17++) {
							...
							if (k17 >= 1 && k17 < regionSizeY - 1
									&& (!lowMem || (tileFlags[0][l6][k17] & 2) != 0
											|| (tileFlags[z][l6][k17] & 0x10) == 0
													&& getCollisionPlane(k17, z, l6) == anInt131)) {
								if (z < maximumPlane)
									maximumPlane = z;
								...
								if (l18 > 0 || i19 > 0) {
									...
									if (i19 == 0) {
										scene.addTile(z, l6, k17, 0, 0, -1, j19, k19, l19, i20, method187(j21, j20),
												method187(j21, k20), method187(j21, l20), method187(j21, i21), 0, 0, 0,
												0, i22, 0);
									} else {
										...
										if (textureId >= 0) {
											...
											j23 = encode(overlay_flo.hue, overlay_flo.saturation,
													overlay_flo.luminance);
											minimapColor = Rasterizer3D.hslToRgb[checkedLight(overlay_flo.hsl16, 96)];
										}

										if (minimapColor == 0x000000 && overlay_flo.anotherRgb != -1) {
											int newMinimapColor = encode(overlay_flo.anotherHue,
										...
										scene.addTile(z, l6, k17, k22, byte4, textureId, j19, k19, l19, i20,
												method187(j21, j20), method187(j21, k20), method187(j21, l20),
												method187(j21, i21), checkedLight(j23, j20), checkedLight(j23, k20),
												checkedLight(j23, l20), checkedLight(j23, i21), i22, minimapColor);
									}
								}
							}
						}
	...
	private void renderObject(int y, SceneGraph scene, CollisionMap class11, int type, int z, int x, int id, int j1) {
		...
		if (type == 2) {
			...
			scene.addWallObject(anIntArray152[j1], ((Renderable) (obj11)), key, y, config, x, ((Renderable) (obj12)),
					mean, anIntArray152[i3], z);
			if (definition.occludes)
				if (j1 == 0) {
					anIntArrayArrayArray135[z][x][y] |= 0x249;
					anIntArrayArrayArray135[z][x][y + 1] |= 0x492;
		...
		if (type == 4) {
			...
			scene.addWallDecoration(key, y, j1 * 512, z, 0, mean, ((Renderable) (obj7)), x, config, 0,
					anIntArray152[j1]);
			return;
		}
		if (type == 5) {
			...
			scene.addWallDecoration(key, y, j1 * 512, z, COSINE_VERTICES[j1] * i4, mean, ((Renderable) (obj13)), x,
					config, SINE_VERTICIES[j1] * i4, anIntArray152[j1]);
			return;
		}
		if (type == 6) {
			Object obj8;
			...
			for (int i3 = 0; i3 < 64; i3++) {
				for (int j3 = 0; j3 < 64; j3++)
					if (l2 == i && i3 >= i1 && i3 < i1 + 8 && j3 >= j1 && j3 < j1 + 8)
						method181(l1 + ChunkUtil.method156(j3 & 7, j, i3 & 7), 0, stream,
								l + ChunkUtil.method155(j, j3 & 7, i3 & 7), k1, j, 0);
					else
	...
	/**
	 * Returns the plane that actually contains the collision flag, to adjust
	 * for objects such as bridges. TODO better name
	 *
	 * @param x
	 *            The x coordinate.
	 * @param y
	...
	public final void method183(CollisionMap aclass11[], SceneGraph worldController, int i, int j, int k, int l,
	...
	private static int smoothNoise(int x, int y) {
		int corners = calculateNoise(x - 1, y - 1) + calculateNoise(x + 1, y - 1) + calculateNoise(x - 1, y + 1)
				+ calculateNoise(x + 1, y + 1);
		int sides = calculateNoise(x - 1, y) + calculateNoise(x + 1, y) + calculateNoise(x, y - 1)
	...
	public static void placeObject(SceneGraph worldController, int i, int j, int k, int l, CollisionMap class11,
		...
		if (k == 4) {
			...
			worldController.addWallDecoration(i3, j, i * 512, k1, 0, l2, ((Renderable) (obj7)), i1, byte1, 0,
					anIntArray152[i]);
			return;
		}
		if (k == 5) {
			...
			worldController.addWallDecoration(i3, j, i * 512, k1, COSINE_VERTICES[i] * j4, l2, ((Renderable) (obj13)),
					i1, byte1, SINE_VERTICIES[i] * j4, anIntArray152[i]);
			return;
		}
		if (k == 6) {
			Object obj8;
