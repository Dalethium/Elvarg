...
public class Rasterizer2D extends Cacheable {
	/**
	 * Sets the Rasterizer2D in the upper left corner with height, width and
	 * pixels set.
	 * 
	 * @param height
	 *            The height of the drawingArea.
	 * @param width
	 *            The width of the drawingArea.
	 * @param pixels
	 *            The array of pixels (RGBColours) in the drawingArea.
	 * @param depth
	 *            An array of fog depths.
	 */
	 ...
	 * 
	 * @param leftX
	 *            The left edge X-Coordinate of the box.
	 * @param topY
	 *            The top edge Y-Coordinate of the box.
	 * @param width
	 *            The width of the box.
	 * @param height
	 *            The height of the box.
	 * @param topColour
	 *            The top rgbColour of the gradient.
	 * @param bottomColour
	 *            The bottom rgbColour of the gradient.
	 * @param opacity
	 *            The opacity value ranging from 0 to 256.
	 */
	public static void drawTransparentGradientBox(int leftX, int topY, int width, int height, int topColour,
			int bottomColour, int opacity) {
		...
		for (int rowIndex = 0; rowIndex < height; rowIndex++) {
			...
			int gradientColour = ((topColour & 0xff00ff) * gradient + (bottomColour & 0xff00ff) * inverseGradient
					& 0xff00ff00)
					+ ((topColour & 0xff00) * gradient + (bottomColour & 0xff00) * inverseGradient & 0xff0000) >>> 8;
			int transparentPixel = ((gradientColour & 0xff00ff) * opacity >> 8 & 0xff00ff)
					+ ((gradientColour & 0xff00) * opacity >> 8 & 0xff00);
			for (int columnIndex = 0; columnIndex < width; columnIndex++) {
				int backgroundPixel = pixels[pixelIndex];
				backgroundPixel = ((backgroundPixel & 0xff00ff) * transparency >> 8 & 0xff00ff)
						+ ((backgroundPixel & 0xff00) * transparency >> 8 & 0xff00);
	...
	/**
	 * Sets the drawingArea to the default size and position. Position: Upper
	 * left corner. Size: As specified before.
	 */
	 ...
	 *            The opacity value ranging from 0 to 256.
	 */
	public static void drawTransparentBoxOutline(int leftX, int topY, int width, int height, int rgbColour,
			int opacity) {
	 ...
	 *            The opacity value ranging from 0 to 256.
	 */
	public static void drawTransparentHorizontalLine(int xPosition, int yPosition, int width, int rgbColour,
			int opacity) {
		if (yPosition < topY || yPosition >= bottomY) {
			return;
		}
		...
		for (int i = 0; i < width; i++) {
			...
			final int transparentColour = (red + otherRed >> 8 << 16) + (green + otherGreen >> 8 << 8)
					+ (blue + otherBlue >> 8);
			pixels[pixelIndex++] = transparentColour;
		}
	}
	 ...
	 *            The opacity value ranging from 0 to 256.
	 */
	public static void drawTransparentVerticalLine(int xPosition, int yPosition, int height, int rgbColour,
			int opacity) {
		if (xPosition < leftX || xPosition >= bottomX) {
			return;
		}
